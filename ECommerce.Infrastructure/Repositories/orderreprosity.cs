    ...
    public class orderreprosity : Iorderreprosity
    {
        ...
        public async Task<orderresponsedto> createorder(createorderdto orderDto)
        {
            ...
            try
            {
                ...
                var order = new order {
                    ...
                    statue = OrderStatus.Pending.ToString(),
                ...
                foreach (var suborder in orderDto.SubOrders)
                {
                    var subOrder = new SubOrder
                    {
                        ...
                        Status = SubOrderStatus.Pending.ToString(),
                ...
                if (calculatedorder.Items != null && calculatedorder.Items.Count > 0)
                {
                    ...
                    if (orderDto.CouponId.HasValue)
                    {
                       ...
                       if (currentDate < coupon.startAt || currentDate > coupon.endAt)
                        ...
                        // Calculate discount
                        if (coupon.DiscountType == CouponType.Fixed.ToString())
                        {
                            discountAmount = coupon.DiscountPercentage.Value;
                        }
                        else if (coupon.DiscountType == CouponType.Percentage.ToString())
                        {
        ...
        public async Task<(bool success, string message)> canselorder(int orderid)
        {
            ...
            try
            {
                ...
                if (order.statue != OrderStatus.Pending.ToString() && order.statue != OrderStatus.Processing.ToString())
                    return (false, $"Cannot cancel order with status '{order.statue}'");
                order.updatedAt = DateTime.Now;
                order.statue = OrderStatus.Canceled.ToString();
                foreach (var suborder in order.SubOrders)
                {
                    suborder.Status = SubOrderStatus.Canceled.ToString();
                    //restore inventory
        ...
        public async Task<(bool success, string message)> cansledsuborder(int suborderid)
        {
            ...
            try
            {
                ...
                if (suborder.Status != SubOrderStatus.Pending.ToString() && suborder.Status != SubOrderStatus.Processing.ToString())
                ...
                if (allsuborders.All(s => s.Status == SubOrderStatus.Canceled.ToString()))
                {
